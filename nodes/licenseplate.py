        ...
        try:
            ...
            if len(all_contours) == 2:
                ...
                if total_area > 6250 and total_area < 7250 and len(contours) >= 2:
                    ...
                    char0 = np.argmax(self.character_model.predict(chars[0].reshape((1, 40, 50, 1)))[0])
                    char1 = np.argmax(self.character_model.predict(chars[1].reshape((1, 40, 50, 1)))[0])
                    char2 = np.argmax(self.character_model.predict(chars[2].reshape((1, 40, 50, 1)))[0])
                    char3 = np.argmax(self.character_model.predict(chars[3].reshape((1, 40, 50, 1)))[0])
